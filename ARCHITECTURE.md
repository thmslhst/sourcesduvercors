...
Observations and confirmations created offline are appended to an **outbox** in IndexedDB with a client-generated UUID and `observed_at` set to creation time, then replayed to the API when connectivity returns. **(decided, Phase 3)** Replay is client-side only — triggered on `online`, window focus/visibility, and a slow interval — rather than the SW Background Sync API: iOS has no Background Sync, so the focus path had to exist anyway, and one code path beats two. Replay stops on 401 (needs sign-in) and network/5xx (retry later); a terminal 4xx marks the item and moves past it, so one refusal never wedges the queue.

**A flush never deletes a contribution (decided 2026-08-10).** A terminal 4xx used to remove the item and report the loss in a banner whose only control was "Ignorer" — the hiker was told about a loss after the fact, with no way to inspect or act on it, and a queue built over a whole hike could go in one tap. Refused items are now *kept*, stamped with `failure: { reason, at, status }`, skipped by later flushes (so they neither wedge the queue nor re-hammer the server), and listed in `components/BlockedSheet.tsx` with what they said, where, when, and why they bounced. Only the hiker removes one. Retry is offered exactly where retrying could honestly change the answer (`rejected`) and withheld where the refusal is a settled fact (`too_old`, `own_observation`, `retired`), so the sheet never shows a control that does nothing.

- The client UUID makes replay **idempotent** (server upserts by ID).
- **One queued reaction per observation.** Observations are keyed in the outbox by their own UUID — each is a distinct append-only fact, so several may queue for one source. A reaction is keyed by its *observation* instead (`reaction:<observationId>`), carrying its UUID in a separate field: the server upserts on (observation, user), so only the last tap can ever apply. Without the derived key, re-tapping stacked superseded writes and inflated the pending count. A reaction queued as a `dispute` before that half was retired is refused on replay rather than delivered to the confirm endpoint, which would invert it.
- **The device remembers what it filed** (`lib/offline/authored.ts`). `GET /api/v1/sources/:id` answers `isMine` from the session, so a signed-out viewer is told `false` for *every* observation, including their own. The sheet therefore offered "Confirmer" on the hiker's own reading; the tap queued behind the missing session, and the sign-in that was meant to send the queue got it refused with `own_observation` instead. Observation ids submitted from this device are mirrored to localStorage — they survive sign-out and reload — and OR'd into `isMine` before `sourcePromptState` decides. A heuristic (a borrowed phone loses one confirm offer) that errs the safe way: it only ever withholds a tap the server would have refused.
